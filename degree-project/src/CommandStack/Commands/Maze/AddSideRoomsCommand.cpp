...
Command::AddSideRoomsCommand::AddSideRoomsCommand(LevelGenerator::LevelStateData& StateData, std::array<DirectionType, 4> RandomDirections, bool AddAdditionalRoom)
	: StateData(StateData),
...
void Command::AddSideRoomsCommand::Execute()
{
	...
	{
		if (LevelGenerator::LevelCell* SideRoom = TryGenerateSideRoom(Cell))
		{
			if (AddAdditionalRoom)
			{
				// And are able to generate another room
				if (LevelGenerator::LevelCell* AdditionalRoom = TryGenerateSideRoom(SideRoom))
				{
					for (const DirectionType Direction : LevelGenerator::NavigationalDirections::GetDirections())
					{
						// Get Neighbor
						LevelGenerator::LevelCell* SideRoomNeighborCell = StateData.GetNeighborCell(AdditionalRoom, Direction);
			...
			{
				LevelGenerator::LevelCell* SideRoomNeighborCell = StateData.GetNeighborCell(SideRoom, Direction);
...
void Command::AddSideRoomsCommand::Undo()
{
	...
	for (const auto& [SideRoom, InsertionIndex] : AddedSideRooms)
	{
		for (const DirectionType Direction : LevelGenerator::NavigationalDirections::GetDirections())
		{
			...
			{
				LevelGenerator::LevelCell* Neighbor = StateData.GetNeighborCell(SideRoom, Direction);
...
LevelGenerator::LevelCell* Command::AddSideRoomsCommand::TryGenerateSideRoom(LevelGenerator::LevelCell* CurrentCell)
{
	...
	{
		LevelGenerator::LevelCell* SideRoomCell = StateData.GetNeighborCell(CurrentCell, Direction);
